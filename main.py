...
#sys.stdout = open('consoleoutput.log', 'a') #redirect console output to file for post mortem

print('=== START ===')

starttime = datetime.now() #record time the experiment was started
allocated_time = 150 #for 3h experiment window set this to 10000 (800 seconds less than 3h)
allocated_space = 2800000000 #size limit of 2.8 Gb (200mb less than 3 Gb) 2800000000
...
"""
@IT.job(interval=timedelta(seconds=1), num_executions=1, initial_delay=timedelta(seconds=1))
def initialize_magnetometer():
    print('mag_cal executed: {}'.format(time.ctime()))
    mag_cal() #cal function that reads magnetometer data 100 times
"""
...
def take_picture():
    ...
    try:
        with open(camlog, 'a') as f:
            print('image capture took : ' + str(tb-ta))
            f.write('image capture took : ' + str(tb-ta) + '\n')
    except Exception as error:
        print('Error writing to camlog: ' + str(error))

take_picture.counter = 0
...
def check_size():
    ...
    total_size = img_size + data_size + log_size
    ...
    if total_size > allocated_space:
        print('Size imit reached - triggering signal')
        signal.raise_signal(signal.SIGUSR1)


"""
@IT.job(interval=timedelta(seconds=20))
def sample_job_every_20s():
    print('20s job current time : {}'.format(time.ctime()))

"""

if __name__ == '__main__': #main function
    try:
        mag_cal(100) #cal function that reads magnetometer data 100 times
    except Exception as error:
...
"""
#delete variables, that are no longer used by the program 
all_variables = dir()
...
for obj in all_variables:
    ...
    if not obj.startswith('__'):
        objvalue = eval(obj)
        print(obj, "is", type(objvalue), "and is equal to ", objvalue)
        del globals()[obj]
...
###############################
# ASTRO PI: PRE-ANALYSIS INIT #
###############################

im_analysis_dir = 'lowimg'
df_brightness = pd.DataFrame(
    columns = ['nr', 'name', 'path', 'brightness']
)
...
#Load magnetic field data into Pandas DataFrames
df_mag = pd.read_csv('data/mag_data.csv')
df_mag.columns = [
    'date', 
    'mag_x', 
    'mag_y', 
...
#Load camera data into Pandas DataFrame
df_cam = pd.read_csv('data/cam_data.csv')
df_cam.columns = [
    'time',
    'nr',
    'lat',
...
#calculate the total strength of the magnetic field for each datapoint
df_mag['strength'] = (df_mag['mag_x']**2 + df_mag['mag_y']**2 + df_mag['mag_z']**2)**0.5

#calculate the downward portion of the magnetic field
df_mag['zcomp'] = df_mag['mag_z'] / df_mag['strength']

#go through all images (at low resolution) and determine average brightness
for file in os.listdir(im_analysis_dir):
    try:
        fnr = int(file[5:-4]) #extract number of image file
    except Exception as error:
        print('File name not recognized: ' + str(error))
        continue
    path = im_analysis_dir + '/' + file
    try:
        b = calc_brightness(path)
        df_brightness.loc[len(df_brightness)] = [fnr, file, path, b]
    except Exception as error:
